...
class Home extends React.Component {
  ...
  componentDidMount () {
    ...
    if (groups.length > 0) {
      ...
      groups.forEach(g => {
        if (g.parentId && groupDic[g.parentId]) {
      ...
      if (accounts.length > 0) {
        ...
        Object.keys(group2Accounts).forEach(groupId => {
          if (group2Accounts[groupId].length > 1) {
            group2Accounts[groupId].sort((a, b) => a.sort - b.sort)
          }
        })
      }
    }
  ...
  componentWillUnmount () {
    ...
    if (sortedGroup.length > 0) {
      sortedGroup.forEach(groupId => {
        if (groupId in group2Accounts) {
          group2Accounts[groupId].forEach((account, index) => {
            if (account.sort !== index) {
              account.sort = index
